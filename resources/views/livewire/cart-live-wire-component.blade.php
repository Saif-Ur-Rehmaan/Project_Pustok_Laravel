        ...
        @forelse ($Books as $item)
            ...
            <div class="cart-product">
                <a href="{{ URL('product-details',Crypt::encrypt($item['book']->id)) }}" class="image">
                    <img src="{{ URL($item['book']->image) }}"
                        alt="{{$item['book']->title}}">
                </a>
                <div class="content">
                    <h3 class="title"><a href="{{ URL('product-details',Crypt::encrypt($item['book']->id)) }}">{{$item['book']->title}}</a></h3>
                    <p class="price"><span class="qty">{{$item['quantity']}} ×</span> 
                    @php
                        if ($item['book']->discountPercent != 0){
                            echo '$'. number_format(($item['book']->priceInUSD * (1 - $item['book']->discountPercent / 100)), 2)  ;
                        }else{
